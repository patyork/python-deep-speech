...
class BRNN:
    def __init__(self, input_dimensionality, output_dimensionality, params=None, learning_rate=.01, momentum_rate=.25, data_x=None, data_y=None):
        ...
        if params is None:
            ...
            self.rf = RecurrentLayer(self.ff3.output, 500, 200, False)     # Forward layer
            self.rb = RecurrentLayer(self.ff3.output, 500, 200, True)      # Backward layer
            self.s = SoftmaxLayer(T.concatenate((self.rf.output, self.rb.output), axis=2), 2*200, output_dimensionality)
        else:
            ...
            self.rf = RecurrentLayer(self.ff3.output, 500, 200, False, params[3])     # Forward layer
            self.rb = RecurrentLayer(self.ff3.output, 500, 200, True, params[4])      # Backward layer
            self.s = SoftmaxLayer(T.concatenate((self.rf.output, self.rb.output), axis=2), 2*200, output_dimensionality, params[5])
...
class Network:
    ...
    def load_network(self, path, input_dimensionality, output_dimensionality, learning_rate=0.001, momentum=.99, datax=None, datay=None):
        f = file(path, 'rb')
        parameters = []
