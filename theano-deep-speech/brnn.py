...
class BRNN:
    def __init__(self, input_dimensionality, output_dimensionality, params=None, learning_rate=.01, momentum_rate=.25, data_x=None, data_y=None):
        ...
        if params is None:
            ...
            self.s = SoftmaxLayer(T.concatenate((self.rf.output, self.rb.output), axis=2), 2*300, output_dimensionality)
        else:
            ...
            self.s = SoftmaxLayer(T.concatenate((self.rf.output, self.rb.output), axis=2), 2*300, output_dimensionality, params[5])
        ...
        self.tester = theano.function(
            inputs=[inputs],
            outputs=[self.rf.output, self.rb.output, self.s.output]
        )
